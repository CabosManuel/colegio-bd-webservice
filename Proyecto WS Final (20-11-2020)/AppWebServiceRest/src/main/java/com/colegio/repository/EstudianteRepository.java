...
public interface EstudianteRepository extends CrudRepository<Estudiante, String> {
	...
	@Query(value = "update estudiantes set nombre = ?1, apellido = ?2, celular = ?3, "
			+ "fnacimiento = ?4, correo = ?5, distrito_id = ?6, direccion = ?7 where dni_estudiante = ?8", nativeQuery = true)
	...
	@Modifying
	@Query(value = "update estudiantes set estado = ?1 where dni_estudiante = ?2", nativeQuery = true)
