...
public interface AsistenciaRepository extends CrudRepository<Asistencia, Integer> {
	...
	// entre semana del primer día del mes, hasta la semana del último día de ese
	// mes
	// agrupado por día,porque en un día hay más de un curso
	@Query(value = "select a.estado, convert(a.asistencia,date) " + "from asistencias a " + "where week(a.asistencia) "
			+ "between week(?2) and (week(last_day(?2))) " + "and a.dni_estudiante like ?1 "
			+ "group by dayofyear(convert(a.asistencia,date))", nativeQuery = true)
	Collection<Object[]> getAsistenciasByDniEstudianteFecha(String dniEstudiante, String fecha);

	@Query(value = "select a.estado, convert(a.asistencia,date) ,hd.curso_id " + "from asistencias a "
			+ "inner join horario_detalle hd " + "on hd.horario_detalle_id = a.horario_detalle_id "
	...
	@Query(value = "select distinct month(a.asistencia) " + "from asistencias a "
	...
	@Modifying
	@Query(value = "insert into asistencias (asistencia, estado, dni_estudiante, horario_detalle_id)"
