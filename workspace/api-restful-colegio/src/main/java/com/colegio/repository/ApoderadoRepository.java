...
public interface ApoderadoRepository extends CrudRepository<Apoderado, String> {
	...
	@Query(value = "SELECT CONCAT(a.nombre, \" \", a.apellido) AS apoderado " + "FROM apoderados a "
	...
	@Modifying
	@Query(value = "update apoderados set nombre = ?1, apellido = ?2, celular = ?3, correo = ?4, distrito_id = ?5, direccion = ?6 where dni_apoderado = ?7", nativeQuery = true)
	...
	@Modifying
	@Query(value = "update apoderados set estado = ?1 where dni_apoderado = ?2", nativeQuery = true)
