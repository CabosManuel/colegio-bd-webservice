...
public interface TrabajadorRepository extends CrudRepository<Trabajador, Integer> {
	...
	@Query(value = "select t.nombres, t.apellidos, t.celular, t.correo, t.sexo, c.nombre as curso "
			+ "from trabajadores t " + "inner join trabajador_curso tc on tc.trabajador_id = t.trabajador_id "
			+ "inner join cursos c on c.curso_id = tc.curso_id " + "inner join mallas mll on mll.curso_id = c.curso_id "
			+ "inner join grados g on g.grado_id = mll.grado_id "
			+ "inner join matriculas mtr on mtr.grado = g.grado_id "
			+ "inner join estudiantes e on e.dni_estudiante = mtr.dni_estudiante " + "where t.cargo = 'docente' and "
			+ "e.dni_estudiante = ? and " + "year(mtr.fecha) = 2020", nativeQuery = true)
	...
	@Query(value = "select t.trabajador_id, t.nombres, t.apellidos, t.cargo, t.dni, t.fecha_nacimiento, "
			...
			+ "  inner join distritos d on t.distrito_id = d.distrito_id " + " where t.dni = ?", nativeQuery = true)
	Map<String, ?> buscarTrabajador(String dniApoderado);

	@Modifying
	@Query(value = "update trabajadores set estado = ?1 where dni = ?2", nativeQuery = true)
