...
public interface NotasRepository extends CrudRepository<Notas, Integer> {
	...
	Collection<Object[]> findByDniEstudianteAnio(String dniEstudiante, String anio);
	...
	Collection<Object> getAniosByDniEstudiante(String dniEstudiante);
	...
	Collection<Notas> findByDniEstudiante(String dniEstudiante);
	...
	Collection<Map<String, ?>> buscarNotassPorCurso(Integer cursoId, Integer seccionId);
	...
	@Modifying
	@Query(value = "insert into notas" + "(seccion_id, curso_id, dni_estudiante, nota1, fecha) values "
			+ "(?1, ?2, ?3, ?4, ?5)", nativeQuery = true)
	void registrarNotas(Integer seccionId, Integer cursoId, String dniEstudiante, Integer nota1, String fecha);

	@Modifying
	@Query(value = "update notas set fecha = ?1, nota1 = ?2, nota2 = ?3, nota3 = ?4 "
			+ "where nota_id = ?5", nativeQuery = true)
	void modificarNotas(String fecha, Integer nota1, Integer nota2, Integer nota3, Integer notaId);
}
